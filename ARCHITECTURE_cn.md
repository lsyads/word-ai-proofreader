...
本文面向开发者和 AI coding agent，说明当前 V2.2 出版审校工作台的代码入口、模块边界和端到端数据流。产品目标和协作约束以 [AGENTS_cn.md](AGENTS_cn.md) 为准；API 契约以 [spec_cn.md](spec_cn.md) 为准。
...
- 修改 API 字段时，同时检查 `backend/app/schemas.py`、`backend/app/main.py`、`word-addin/src/taskpane/types.ts`、`word-addin/src/taskpane/api.ts` 和 [spec_cn.md](spec_cn.md)。
...
需要验证当前行为时，优先读源码、schema、测试和 [spec_cn.md](spec_cn.md)，不要从上述生成物反推契约。
