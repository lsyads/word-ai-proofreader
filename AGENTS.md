...
完成一个可运行、可测试、可展示的出版审校 Agent 工作台，使责任编辑不只是“调用一次 AI 审校”，而是可以围绕一本书建立审校项目，让 Agent 制定计划、分阶段执行、跨章节追踪问题、给出证据、复核候选结果，并在编辑确认后写回 Word。
...
3. 建立真正的 Agent 编排层：
   - 使用 LangGraph 显式定义 planner、reviewer、evaluator、writer 等可观测节点。
   - 不使用黑盒式“一个超级 prompt 跑到底”的实现。
   - 不把所有逻辑塞进单个 FastAPI endpoint。
   - Agent 可以根据审校目标选择工具，但工具边界、输入输出和失败处理必须清晰。

4. 建立出版审校工具注册表：
   - 将文档解析、文档地图构建、分块、AI 审校、术语检查、体例检查、跨章节一致性检查、问题归并、证据绑定、DOCX 写回等能力封装为 tool 或可被 tool 调用的 service function。
   - tool 输入输出必须有明确类型。
