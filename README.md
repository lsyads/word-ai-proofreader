...
当前代码已升级到 V2.1 工作台闭环：创建当前选区或 DOCX 审校项目、建立文档地图、生成审校计划、后台运行分阶段 Agent、让审校目标进入 V2 prompt、执行基础审校/术语一致性/体例规则/跨章节一致性/evaluator pass、生成带证据和置信度的候选问题、批量批准/拒绝/暂缓、沉淀轻量项目记忆、写回已批准问题、下载 DOCX 结果、查询按 pass 汇总的审校报告和脱敏 run event trace。插件 UI 不再提供 V1 独立入口；当前选区写回由 Office.js 完成，DOCX 写回由后端完成。V2.1 不迁移 V1/V2 旧历史、trace、任务状态或 DOCX result index。
